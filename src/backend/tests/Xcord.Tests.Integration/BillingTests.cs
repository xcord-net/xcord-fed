...
public class BillingTests
{
    ...
    public async Task GetBillingConfig_AsOwner_ReturnsConfig()
    {
        ...
        var body = await response.ReadAsJsonAsync<JsonElement>();
        body.GetProperty("serverId").ReadLong().Should().Be(ctx.ServerId);
        body.GetProperty("revenueSharePercent").GetInt32().Should().Be(70);
        body.GetProperty("payoutEnabled").GetBoolean().Should().BeFalse();
        body.GetProperty("isConnected").GetBoolean().Should().BeFalse(
            "no Stripe connected account in test environment");
    }
