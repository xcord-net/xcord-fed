...
/// <summary>
/// Admin-only endpoint to rotate the AES-GCM Data Encryption Key (DEK).
    ...
    : IRequestHandler<RotateDataKeyCommand, Result<RotateDataKeyResponse>>
{
        ...
        RotateDataKeyCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            logger.LogInformation("DEK rotation completed; new active version is {Version}", newVersion);
            return new RotateDataKeyResponse(newVersion);
        }
        catch (InvalidOperationException ex)
        {
            logger.LogWarning(ex, "DEK rotation rejected: {Reason}", ex.Message);
            return Error.Failure("KEY_ROTATION_UNAVAILABLE", ex.Message);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "DEK rotation failed");
