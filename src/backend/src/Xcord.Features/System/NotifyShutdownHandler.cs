...
/// drops.  This endpoint is internal-only and is protected by the InternalKey policy
/// (X-Internal-Key shared secret) rather than user authentication.
/// </summary>
    ...
    : IRequestHandler<NotifyShutdownRequest, Result<bool>>
{
    public async Task<Result<bool>> Handle(NotifyShutdownRequest request, CancellationToken cancellationToken)
    {
        ...
        await broadcaster.BroadcastAsync("System_ShuttingDown", payload, cancellationToken).ConfigureAwait(false);
