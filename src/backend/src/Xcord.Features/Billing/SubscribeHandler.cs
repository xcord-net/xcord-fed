    ...
    : IRequestHandler<SubscribeCommand, Result<SubscribeResponse>>
{
        ...
        SubscribeCommand request, CancellationToken cancellationToken)
    {
        ...
        // If this instance is configured with its own Stripe account, create a hosted checkout.
        // Otherwise (dev/self-hosted without Stripe), apply the subscription directly.
        if (billingOptions.Value.IsConfigured)
        {
